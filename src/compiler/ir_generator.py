...
def generate_ir(root_node: ast.Module) -> dict[str, list[ir.Instruction]]:
    ...
    def visit(
        st: SymTab,
        expr: ast.Expression,
        start_label: ir.Label | None = None,
        end_label: ir.Label | None = None,
    ) -> ir.IRVar:
        loc = expr.location
        ...
        match expr:
            case ast.Literal():
                match expr.value:
                    case bool():
                        var = new_var(Bool)
                        instructions.append(
                            ir.LoadBoolConst(location=loc, value=expr.value, dest=var)
                        )
                    case int():
                        var = new_var(Int)
                        instructions.append(
                            ir.LoadIntConst(location=loc, value=expr.value, dest=var)
                        )
                    case None:
                        var = new_var(Unit)
                    case _:
            ...
            case ast.IfExpression():
                if expr.else_clause is None:
                    ...
                    instructions.append(
                        ir.Copy(location=loc, src=var_then, dest=var_result)
                    )
                    ...
                    instructions.append(
                        ir.Copy(location=loc, src=var_else, dest=var_result)
                    )
                    instructions.append(ir.Label(location=loc, name=l_end.name))
            ...
            case ast.BinaryLogical():
                ...
                if expr.op == "and":
                    ...
                    var_right = visit(st, expr.right, start_label, end_label)
                    instructions.append(
                        ir.Copy(location=loc, src=var_right, dest=var_result)
                    )
                    instructions.append(ir.Jump(location=loc, label=l_end))

                    instructions.append(ir.Label(location=loc, name=l_skip.name))
                    instructions.append(
                        ir.LoadBoolConst(location=loc, value=False, dest=var_result)
                    )
                    instructions.append(ir.Jump(location=loc, label=l_end))
                    ...
                    instructions.append(ir.Label(location=loc, name=l_skip.name))
                    instructions.append(
                        ir.LoadBoolConst(location=loc, value=True, dest=var_result)
                    )
                    instructions.append(ir.Jump(location=loc, label=l_end))
            ...
            case ast.Function():
                ...
                args = [
                    visit(st, arg, start_label, end_label) for arg in expr.arguments
                ]
                dest = new_var(expr.type)
                instructions.append(
                    ir.Call(location=loc, fun=fun, args=args, dest=dest)
                )
            ...
            case ast.While():
                ...
                instructions.append(
                    ir.CondJump(
                        location=loc, cond=var_cond, then_label=l_body, else_label=l_end
                    )
                )
            ...
            case ast.Variable():
                if expr.ident.name in st.locals:
                    raise Exception(
                        f"Variable already defined in this scope: {expr.ident.name}"
                    )
            ...
            case ast.UnaryOp():
                ...
                instructions.append(
                    ir.Call(
                        location=loc, fun=var_op, args=[var_operand], dest=var_result
                    )
                )
                return var_result
    ...
    if var_types[var_final_result] == Int:
        instructions.append(
            ir.Call(
                location=None,
                fun=root_symtab.locals["print_int"],
                args=[var_final_result],
                dest=new_var(Unit),
            )
        )
        ...
        instructions.append(
            ir.Call(
                location=None,
                fun=root_symtab.locals["print_bool"],
                args=[var_final_result],
                dest=new_var(Unit),
            )
        )
