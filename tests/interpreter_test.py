...
def test_interpreter_binary_comp() -> None:
    assert interpret(node=parse(tokenize("2 < 3"))) is True
    assert interpret(node=parse(tokenize("2 > 3"))) is False
    assert interpret(node=parse(tokenize("2 <= 3"))) is True
    assert interpret(node=parse(tokenize("2 >= 3"))) is False
    assert interpret(node=parse(tokenize("2 == 3"))) is False
    assert interpret(node=parse(tokenize("2 != 3"))) is True
...
def test_interpreter_binary_logical() -> None:
    assert interpret(node=parse(tokenize("true and false"))) is False
    assert interpret(node=parse(tokenize("true or false"))) is True
    return None


def test_interpreter_unary_op() -> None:
    assert interpret(node=parse(tokenize("var x = 2; -x"))) == -2
    assert interpret(node=parse(tokenize("not true"))) is False
    return None
...
def test_interpreter_short_circuiting() -> None:
    assert (
        interpret(node=parse(tokenize("var e = false; true or { e = true; true }; e")))
        is False
    )
    return None
...
def test_interpreter_while() -> None:
    assert (
        interpret(node=parse(tokenize("var x = 0; while x < 3 do { x = x + 1 }; x")))
        == 3
    )
    return None
...
def test_interpreter_break() -> None:
    assert (
        interpret(
            node=parse(
                tokenize(
                    "var x = 0; while true do { x = x + 1; if x == 3 then break }; x"
                )
            )
        )
        == 3
...
def test_interpreter_continue() -> None:
    assert (
        interpret(
            node=parse(
                tokenize(
                    "var x = 0; while x < 3 do { x = x + 2; if x == 2 then continue; x = x + 1;} x"
                )
            )
        )
        == 5
...
def test_interpreter_function_def() -> None:
    assert (
        interpret(
            node=parse(tokenize("fun square(x: Int): Int { return x * x; } square(2)"))
        )
        == 4
    )
    assert (
        interpret(
            node=parse(tokenize("fun add(x: Int, y: Int): Unit { x + y } add(2, 3)"))
        )
        is None
    )
    return None
