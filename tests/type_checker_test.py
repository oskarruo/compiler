...
def test_type_checker_binary_op() -> None:
    ...
    with pytest.raises(
        Exception,
        match=re.escape(
            "Invalid types for operator +: BasicType(name='Bool'), BasicType(name='Int')"
        ),
    ):
...
def test_type_checker_binary_logical() -> None:
    ...
    with pytest.raises(
        Exception,
        match=re.escape(
            "Invalid types for operator and: BasicType(name='Int'), BasicType(name='Bool')"
        ),
    ):
    ...
    with pytest.raises(
        Exception,
        match=re.escape(
            "Invalid types for operator or: BasicType(name='Int'), BasicType(name='Bool')"
        ),
    ):
...
def test_type_checker_unary_op() -> None:
    assert (
        typecheck(parse(tokenize("-x")), symtab=SymTab({"x": Int}, SymTab({}, None)))
        == Int
    )
    ...
    with pytest.raises(
        Exception,
        match=re.escape("Invalid type for operator not: BasicType(name='Int')"),
    ):
...
def test_type_checker_if_expression() -> None:
    ...
    with pytest.raises(
        Exception,
        match=re.escape(
            "Invalid types for then and else: BasicType(name='Int'), BasicType(name='Bool')"
        ),
    ):
    ...
    with pytest.raises(
        Exception, match=re.escape("Invalid type for condition: BasicType(name='Int')")
    ):
...
def test_type_checker_function() -> None:
    ...
    with pytest.raises(
        Exception, match=re.escape("Invalid type for argument: BasicType(name='Bool')")
    ):
...
def test_type_checker_variable_declaration() -> None:
    ...
    with pytest.raises(
        Exception,
        match=re.escape(
            "Invalid types for variable x: BasicType(name='Bool'), BasicType(name='Int')"
        ),
    ):
...
def test_type_checker_variable_assignment() -> None:
    ...
    with pytest.raises(
        Exception,
        match=re.escape(
            "Invalid types for assignement: BasicType(name='Bool'), BasicType(name='Int')"
        ),
    ):
...
def test_type_checker_function_def() -> None:
    with pytest.raises(
        ...
        typecheck(
            parse(tokenize("fun square(x: Int): Int { return x * x; } square(true);"))
        )

    assert (
        typecheck(
            parse(tokenize("fun square(x: Int): Int { return x * x; } square(42)"))
        )
        == Int
    )
    ...
    assert (
        typecheck(
            parse(
                tokenize(
                    "fun f(x: Int): Int { return g(x + 1); } fun g(x: Int): Int { return x } f(1) * 100"
                )
            )
        )
        == Int
