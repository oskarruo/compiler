...
def test_parser_function_def() -> None:
    assert parse(tokenize('fun square(x: Int): Int { return x * x; } square(2);')) == ast.Module(
        funs=[ast.FunDef(
            location=L(),
            name='square',
            params=[
                ast.FunDefArg(name='x', type=ast.Identifier(name='Int', location=L()))
            ],
            return_type=ast.Identifier(name='Int', location=L()),
            body=ast.Block(
                expressions=[ast.ReturnExpression(value=ast.BinaryOp(
                    left=ast.Identifier(name='x', location=L()),
                    op='*',
        ...
        body=ast.Block(
            expressions=[ast.Function(name='square', arguments=[ast.Literal(value=2, location=L())], location=L())],
            result=ast.Literal(value=None, location=L()),
            location=L()
        )
    )
